...
const ListDownload = () => {
  ...
  const fetchFileList = async () => {
    try {
      const response = await axios.get('http://192.168.20.195:5000/api/download/filelist');
  ...
  const handleDownload = (filename) => {
    ...
    window.location.href = `http://192.168.20.195:5000/api/download/output/${filename}`;
  };
